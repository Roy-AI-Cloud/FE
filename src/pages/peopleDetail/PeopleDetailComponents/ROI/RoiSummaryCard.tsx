...
const RoiSummaryCard = ({ estimateData }: RoiSummaryCardProps) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border p-6">
      <h3 className="text-2xl font-bold mb-4 p-4">ROI 종합 평가</h3>
      ...
      <p className="text-sm text-gray-700 mb-6 p-4">
        {score >= 70
          ? "우수한 ROI 예상"
          : score >= 50
          ? "검토 필요"
      ...
      <div
        className={`p-5 rounded-lg border ${currentGradeColor.summaryBg} ${currentGradeColor.summaryBorder}`}
      >
        <div className="flex flex-wrap gap-4">
          <div>
            <span className="text-sm text-gray-600">예상 조회수</span>{" "}
            <span className="font-semibold">
              {estimateData.estimated_views.toLocaleString()}회
            </span>
          </div>
          <div>
            <span className="text-sm text-gray-600">예상 참여율</span>{" "}
            <span className="font-semibold">
              {estimateData.estimated_engagement}%
            </span>
          </div>
          <div>
